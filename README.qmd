...
We want to know how many males there are at age 1 over 500 lbs.
...
ggplot(knz_bison2, aes(x= age, y = animal_weight, color=animal_sex))+
  geom_point()
...
```{r}
# For 1 through the length of the vector "vec" 
for (i in 1:nrow(knz_bison2)) {
  filter_data <- knz_bison2 %>% 
    filter(age == )
  print(knz_bison[i, "animal_weight"])
